...
/***/ (function(module, exports, __webpack_require__) {
	...
	function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } } //这里是有顺序要求的，因为有些语法需要使用babel-polyfill来处理，所以需要先导入它
	...
	var Lottery = function (_mix) {
		...
		function Lottery() {
			...
			_this.el = '';
			_this.omit = new Map(); //遗漏数据
			_this.open_code = new Set(); //开奖号码
			_this.play_list = new Map(); //玩法说明
			_this.number = new Set(); //开奖号码
			_this.cart_el = '.codelist'; //购物车选择器
			_this.omit_el = ''; //遗漏
			_this.cur_play = 'r5'; //默认玩法
			_this.open_code_list = new Set(); //开奖记录

			_this.initPlayList(); //base.js中实现
			_this.initNumber(); //base.js中实现
			_this.updateState();
			_this.initEvent();
			return _this;
		}
	  ...
	  */
	   ...
	   */
			...
			value: function initEvent() {
				var self = this;
				(0, _jquery2.default)('.boll-list').on('click', '.btn-boll', self.toggleCodeActive.bind(self)); //号码的选中和取消
				...
				(0, _jquery2.default)('.qkmethod').on('click', '.btn-middle', self.getRandomCode.bind(self)); //添加随机号码
			}
		}]);
	...
	var Calculate = function () {
	   ...
	   */
			...
			value: function computeBonus(active, play_name) {
				var play = play_name.split(''); //分成r和数字
				var self = this;
				var arr = new Array(play[1] * 1).fill(0); //与computeCount用法类似
				var min = void 0,
				    max = void 0;
				if (play[0] === 'r') {
